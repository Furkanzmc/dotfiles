...
- When searching for files, use `fd` instead of `find` command.
- When searching for text, use `ug` instead of `grep` command.
- When replacing text, use `sd` instead of `sed` command.
- When listing files, use `exa` instead of `ls` command.
- I want to make use of you so I can learn better, faster, and gain depth of knowledge. So, make
sure to leave comments in the code, give me instructions, ask me questions so we can facilitate
learning.

## C++ Coding Conventions

- Practice extreme programming, always assert for pre/post conditions.
- Use `{}` initializers and `{.field=value}` designated initializers.
...
- When using search algorithms, prefer to use `.cbegin()` and `.cend()`.
- Prefer forward declarations to including a header if the type is allocated on the heap.
- Prefer to use `std::optional` to indicate optional things.
- Prefer to use references instead of pointers, and value types when warranted.
- If a type you are passing as const reference is copied without condition, then prefer to pass
it by value and move it.
- When declaring local variables, always initialize with `{}`, even if they are default constructed.
- When declaring member variables, always initialize with `{}` if the value is not being changed in
  the constructor.
- Use references instead of pointers whenever you can.
- Always assert for `nullptr`.
- For Qt signals, always use private signals like `void propertyChanged( QPrivateSignal )`.
- If a method doesn't require access to more than 3 member variables, prefer to implement it as a pure anonymous function.
- Make aggressive use of `const` and `constexpr`.
- Make aggressive use of `assert`. Always assert for pre-conditions and post-conditions.
- If a member variable is not initialized in the constructor, it should be initialized in the header
  file.
- Prefer to use brace initialization over parentheses initialization.
- `Qt::UniqueConnection` requires a method or a function, it won't work with lambdas.
- Always use `const` variables unless it has to be changed. Also, always prefer `const` methods.
- Do not use `delete` to delete `QObject` pointers. Use `deleteLater()` instead.
- Avoid unnecessary and obvious comments. Comments should explain why something is done, not what is
  done.
- When using `auto`, prefer to use `auto*` for pointers and `auto&` for references if the right hand
...
- Always use `const` or `let` for declaring variables.
