...
local s_completion_sources = {
	...
	{ keys = "<c-x><c-f>" },
	{
		keys = "<c-x><c-k>",
		prediciate = function()
			return pcall(api.nvim_buf_get_option, ".", "dictionary") or pcall(api.nvim_get_option, ".", "dictionary")
		end,
	},
	{
		keys = "<c-x><c-s>",
...
local function timer_handler()
	...
	if vim.fn.pumvisible() == 0 then
		if s_completion_index == #completion_sources + 1 then
			...
			api.nvim_feedkeys(api.nvim_replace_termcodes("<c-g><c-g>", true, false, true), "n", true)
...
function M.setup_completion(bufnr)
	...
	cmd([[au!]])
	cmd("autocmd CompleteDonePre <buffer=" .. bufnr .. ">" .. " lua require'vimrc.completion'.on_complete_done_pre()")
