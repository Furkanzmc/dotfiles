...
function M.open_files(args, use_split)
    ...
    if use_split == true then
        command = table.map(command, function(val, k, t)
            if k == 1 then return "split " .. val end

            return "split " .. val
        end)
    else
        command = table.map(command,
                            function(val, k, t) return "edit " .. val end)
    end
...
function M.complete(arg_lead, cmd_line, cursor_pos)
    ...
    if ok then table.extend(search_paths, fn.split(local_paths, ",")) end
    ...
    if #cmd_args == #fd_args then table.insert(cmd_args, ".") end
    ...
    return table.map(result,
                     function(val, k, t) return fn.fnamemodify(val, ":~:.") end)
end
