...
function M.clean_trailing_spaces()
	if options.get_option_value("clstrailingwhitespace", vim.api.nvim_get_current_buf()) == false then
		return
	end

	local save_cursor = fn.getpos(".")
	local old_query = fn.getreg("/")
	local threshold = options.get_option_value("clstrailingspacelimit", vim.api.nvim_get_current_buf())
	if threshold > 0 then
