...
function pvim() {
    ...
    if ($sessionFile -ne "" -and $rcFile -ne "") {
        if ($Neovide) {
            Start-Process -FilePath neovide -ArgumentList "--fork --no-tabs -- -S $rcFile -S $sessionFile" -Environment @{AW_AUTO_SESSION=1}
        }
        else {
    ...
    elseif ($sessionFile -ne "") {
        if ($Neovide) {
            Start-Process -FilePath neovide -ArgumentList "--fork --no-tabs -- -S $sessionFile" -Environment @{AW_AUTO_SESSION=1}
        }
        else {
    ...
    elseif ($rcFile -ne "") {
        if ($Neovide) {
            Start-Process -FilePath neovide -ArgumentList "--fork --no-tabs -- -S $rcFile" -Environment @{AW_AUTO_SESSION=1}
        }
        else {
        ...
        if ($Neovide) {
            Start-Process -FilePath neovide -ArgumentList "--fork --no-tabs" -Environment @{AW_AUTO_SESSION=1}
        }
        else {
